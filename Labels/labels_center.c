...
//void (*mods[8]) (char* str, uint8_t freq, uint8_t shift) ={Nada,CamelCase,Brnrd,Missing,Leet,Pig,Reverse,Scramble,Caesar};
...
void level_setup(uint8_t lev_num, _LEVEL *level){
	if (lev_num == 1){ // First level has no complications
		...
		pick_labels(level,theme,6);
		level->mod=zero_prob(randint(0,3),50,0};
		...
		level->message=messages[theme];
		pick_labels(level,theme,6);
		level->mod=zero_prob(randint(0,8),30,0};
